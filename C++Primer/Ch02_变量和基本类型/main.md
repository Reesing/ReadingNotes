...
- 引用的定义
    - 除两种<a name="two-exception">例外</a>情况（见 1. [对常量的引用](#const-ref)/[指向常量的指针](#const-ptr)与 2. XXXX）外，
...
- 对常量的引用（reference to const）
    - 对常量的引用亦不能被修改
        ...
        ```
    - **对常量的引用 = 常量引用**
    （但 指向常量的指针 != 常量指针，见XXXX）

- 初始化和对 `const` 的引用
    - [两个例外](#two-exception)之一：<a name="const-ref">**对常量的引用**</a>
        - 在初始化常量引用时允许用**任意结果可以转化为该引用的类型的表达式**作为初始值
    - 若常量引用与初始值类型相同时
        ```cpp
        int i = 42;
        const int &r1 = i;  // 正确，但注意 ri 会随着 i 的变化而变化！
        i = 30;
        std::cout << r1 << std::endl; // 输出30
        int &r2 = r1; // 错误，r1 是 const 引用，但 r2 是非 const 引用
        ```
        - 注意此种情况下，**常量引用会随着被绑定对象（非 `const`）的变化的变化**
        - 但**绝对不允许直接修改常量引用（ `ri` ）所绑定的对象**
    - 若常量引用与初始值类型不同时，会引入**临时量**对象被常量引用绑定
        ```cpp
        // 若运行以下代码：
        double d = 3.14;
        const int &ri = d;
        ...
        - 原初始值对象的改变( `d` )的变化不会引临时量的变化，因此也不会引起常量引用（ `ri` ）的变化
...
- 指向常量的指针（pointer to const）
    ...
    double *ptr = &pi;      // pi 是 const，ptr 是非 const，ptr 不能指向 pi 的地址
    const double *cptr = &pi; // 正确
    *cptr = 42;             // 错误，不能给指向常量的指针赋值
    ```
    - [两个例外](#two-exception)之一：<a name="const-ptr">**指向常量的指针**</a>

        允许指向常量的指针指向一个非常量对象
        ```cpp
        double d = 3.14;
        cptr = &d;       // 正确，但不能通过 cptr 改变 d 的值
        ```

    - 可以这样理解：**所谓指向变量的指针和引用，不过是指针或引用“自以为是”地觉得自己指向了常量罢了，所以会自觉地不去改变所指对象的值**